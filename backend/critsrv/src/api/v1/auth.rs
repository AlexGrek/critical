use crate::{
    auth::invites::use_registration_invite, errors::AppError, middleware::AuthenticatedUser,
    state::AppState,
};
use axum::{
    extract::{Json, State},
...
use crit_shared::{
    entities::User,
    requests::{LoginRequest, LoginResponse, RegisterRequest},
};
...
pub async fn register(
    ...
    log::info!(
        "Auth event -> {}",
        format!("User with ID {:?} created: {}", &req.uid, &req.email)
    );
...
pub async fn login(
    ...
    let user = user_provider
        .try_get_by_key(&req.uid)
        .await?
        .ok_or(AppError::InvalidCredentials)?;

    if !app_state
        .auth
        .verify_password(&req.password, &user.password_hash.unwrap_or("".to_string()))?
    {
