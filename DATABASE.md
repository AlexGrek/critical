...
| ID prefixes | `u_` users · `g_` groups · `sa_` service_accounts · `pa_` pipeline_accounts · no prefix for projects and CRDs (kind name is the key) |
...
| `avatar_ulid` | String? | ULID of the active avatar upload. See `group_avatars/` in object store |
| `wallpaper_ulid` | String? | ULID of the active wallpaper upload. See `group_wallpapers/` in object store |
...
| `avatar_ulid` | String? | ULID of the active avatar upload |
| `wallpaper_ulid` | String? | ULID of the active wallpaper upload |
...
| `credential` | String? | ID of the `repo_credentials` document supplying the secret for `auth_method`. Omitted when absent |
...
|---------|-------|-------------|
| `None` | `"none"` | Anonymous access (plain HTTPS, public repos only) |
| `Ssh` | `"ssh"` | SSH key authentication, using the referenced `RepoCredential` |
| `GithubToken` | `"github_token"` | GitHub personal access token, using the referenced `RepoCredential` |
...
### `repo_credentials` (vertex)

Stored credentials (SSH keys or access tokens) usable by any project's `RepoLink`s via the `credential` field. Kept as a separate ACL'd collection rather than inline on the project so a credential can be reused across repos/projects and read access can be restricted independently of who can edit the project.

Generated by `#[crit_derive::crit_resource(collection = "repo_credentials", prefix = "rc_")]`.

`secret` and `passphrase` are write-only: accepted on create/update, stored in plaintext, and always stripped from API responses (the same pattern as `password`/`password_hash` on `User`). `to_external` adds a computed `has_secret: bool` so callers can tell whether a secret is set without ever reading it back.

> **Known limitation**: secrets are stored in plaintext in ArangoDB. The ACL restricts who can read them *through the API*, but not direct DB access, and the godmode-only `/v1/debug/collections/{name}` route can dump this collection. There is no encryption-at-rest anywhere in this codebase yet.

| Field | Type | Notes |
|-------|------|-------|
| `_key` | String | `rc_`-prefixed ID |
| `labels` | Labels | Queryable key-value pairs |
| `annotations` | Labels | Non-queryable freeform strings |
| `state` | ResourceState | Server-managed audit timestamps |
| `acl` | AccessControlStore | Document-level ACL |
...
| `hash_code` | String | |
| `name` | String | Display name |
| `method` | RepoAuthMethod | `ssh` or `github_token` (see the `RepoLink` section above) |
| `description` | String? | |
| `username` | String? | SSH user; defaults to `"git"` when absent |
| `secret` | String? | Write-only. SSH private key PEM or GitHub token. Never returned by the API |
| `passphrase` | String? | Write-only. Passphrase for an encrypted SSH key. Never returned by the API |

Brief fields: `id`, `labels`, `name`, `method`

Access: same model as `projects` — `ADM_CONFIG_EDITOR` bypass, otherwise document ACL (READ/MODIFY), and `USR_CREATE_PROJECTS` to create.

---

### `crds` (vertex)

Stores Custom Resource Definitions — data-driven schemas that describe new resource kinds the platform can manage dynamically. Each CRD document describes the fields, scope, ACL mode, nouns, and edge relations for a new kind. CRD instances live in a collection named after the CRD's `_key`.

Generated by `#[crit_resource(collection = "crds", prefix = "", no_acl)]`.

Access: any authenticated user may read; only `ADM_GODMODE` may write.

Index: persistent index on `["deletion"]` (same as other global collections).

| Field | Type | Notes |
|-------|------|-------|
| `_key` | String | CRD name (kind identifier), e.g. `deployments`, `ci-runs`. No prefix. 2–63 chars, alphanumeric + `_` + `-`, no leading digit or `-` |
| `labels` | Labels | Queryable key-value pairs (user-managed desired state) |
| `annotations` | Labels | Non-queryable freeform strings (user-managed desired state) |
| `state` | ResourceState | Server-managed audit: `created_at`, `created_by`, `updated_at`, `updated_by` |
| `deletion` | DeletionInfo? | Present = soft-deleted. Absent = active |
| `hash_code` | String | FNV-1a hash of desired state |
| `scope` | CrdScope | Where instances of this resource live: `global` or `project` |
| `acl_mode` | CrdAclMode | How ACLs are applied to instances: `special`, `inherit`, or `custom` |
| `nouns` | CrdNouns | Human-readable noun forms for CLI/UI |
| `relations` | Vec\<CrdRelation\> | Edge relations this resource participates in. Omitted when empty |
| `fields` | Map\<String, FieldDef\> | Named field definitions for instances of this kind |
| `id_prefix` | String | Optional ID prefix for instances, e.g. `"dep_"`. Empty string = no prefix (like projects). Default `""` |
| `super_permission` | String? | Optional super-permission key that short-circuits ACL checks for this kind. Omitted when absent |
| `description` | String? | Human-readable description of this resource kind. Omitted when absent |

Brief fields: `id`, `labels`, `annotations`, `nouns`, `scope`, `acl_mode`, `description`

#### `CrdScope` enum

Controls where instances of this resource live. Serialised as snake_case strings.

| Variant | Value | Description |
|---------|-------|-------------|
| `Global` | `"global"` | Cluster-wide resource, accessed at `/v1/global/{kind}` |
| `Project` | `"project"` | Namespaced under a project, accessed at `/v1/projects/{project}/{kind}` |

#### `CrdAclMode` enum

Controls how ACLs are applied to instances of this resource kind. Serialised as snake_case strings.

| Variant | Value | Description |
|---------|-------|-------------|
| `Special` | `"special"` | No ACL field on instances; access controlled exclusively by super-permissions. Equivalent to `#[crit_resource(no_acl)]` |
| `Inherit` | `"inherit"` | Instances do not carry their own ACL; permission checks fall back to the parent project's ACL (project-scoped only) |
| `Custom` | `"custom"` | Each instance carries its own `AccessControlStore` |
...
| `plural` | String | Plural form, e.g. `"deployments"`. Also used as the API path segment |

#### `CrdRelation` sub-type

| Field | Type | Notes |
|-------|------|-------|
| `edge_collection` | String | ArangoDB edge collection name, e.g. `"deploys_to"` |
| `direction` | RelationDirection | Direction from this resource's perspective: `outbound` (this resource is `_from`) or `inbound` (this resource is `_to`) |
| `target_kind` | String | The kind (CRD name or built-in kind) on the other end |
| `label` | String? | Human label for the relation, e.g. `"targets environment"`. Omitted when absent |

#### `FieldType` enum

Primitive and composite types available for CRD field definitions. Serialised with a `type` tag (snake_case).

| Variant | `type` value | Extra fields | Description |
|---------|-------------|--------------|-------------|
| `String` | `"string"` | — | UTF-8 string |
| `Int` | `"int"` | — | Signed 64-bit integer |
| `Float` | `"float"` | — | 64-bit IEEE 754 float |
| `Bool` | `"bool"` | — | Boolean |
| `Datetime` | `"datetime"` | — | RFC 3339 date-time (stored as ISO 8601 string) |
| `Id` | `"id"` | — | Resource identifier validated with platform ID rules |
| `List` | `"list"` | `items: FieldType` | Ordered list; all elements share the same type |
| `Map` | `"map"` | `values: FieldType` | String-keyed map; all values share the same type |
| `Enum` | `"enum"` | `variants: Vec<String>` | Value must be one of the listed strings |
| `Bitflags` | `"bitflags"` | `bit_positions: Map<String, u8>` | `u64` bitfield; map gives flag name → bit position (0–63). Bit value = `1 << bit_position` |
| `Object` | `"object"` | `fields: Map<String, FieldDef>` | Inline nested object with its own named fields (recursive) |
| `Ref` | `"ref"` | `kind: String` | Reference to another resource by kind + ID; stored as a plain string |

#### `FieldDef` sub-type

A single field definition inside a CRD schema.

| Field | Type | Notes |
|-------|------|-------|
| `type` | (tag from FieldType) | Flattened — the `type` discriminant and any variant-specific fields appear at the same level |
| `required` | bool | When `true`, the field must be present and non-null on create/update. Default `false` |
| `default` | JSON? | Server-side default value applied when the field is absent. Omitted when not set |
| `description` | String? | Human-readable description for docs/UI tooltips. Omitted when absent |
| `brief` | bool | Include this field in the `{Name}Brief` list projection. Default `false` |
| `immutable` | bool | Server rejects changes to this field after creation. Default `false` |
...
### `ticketgroups` (vertex, project-scoped)

JIRA-style issue type schemes scoped to a project. Each group defines one or more ticket types with their own workflow statuses and optional custom fields.

Generated by `#[crit_resource(collection = "ticketgroups", prefix = "tg_")]`.

The resource `id` must be 2-6 capital letters A-Z (e.g. `BUG`, `TASK`, `FEAT`). The controller prefixes it with `tg_` before storing (e.g. `tg_BUG`).

ACL: **hybrid** — resource's own `acl.list` if non-empty; otherwise inherits from parent project ACL (filtered by `scope = "ticketgroups"` or no scope).

| Field | Type | Notes |
|-------|------|-------|
| `_key` | String | `tg_{CODE}`, e.g. `tg_BUG` |
| `labels` | Labels | Queryable key-value pairs |
| `annotations` | Labels | Non-queryable freeform strings |
| `state` | ResourceState | Server-managed audit timestamps |
| `acl` | AccessControlStore | Per-resource ACL (optional — inherits project ACL if empty) |
| `deletion` | DeletionInfo? | Soft-delete marker |
| `hash_code` | String | FNV-1a hash of desired state |
| `name` | String | Display name (e.g. `"Engineering Bugs"`) |
| `description` | String? | What ticket types this group covers |
| `ticket_types` | Vec\<TicketTypeDef\> | Ticket type definitions (see below) |
| `project` | String | Parent project `_key` — injected by scoped handler |

**TicketTypeDef fields:**

| Field | Type | Notes |
|-------|------|-------|
| `name` | String | Type name (e.g. `"Bug"`, `"Story"`) |
| `description` | String? | Optional description |
| `statuses` | Vec\<TicketStatus\> | Ordered workflow statuses |
| `fields` | Vec\<TicketFieldDef\> | Custom fields (omitted = standard fields only) |

**TicketStatus fields:** `name: String`, `category: StatusCategory` (`todo` / `in_progress` / `done`)

**TicketFieldDef fields:** `name`, `field_type` (`text` / `number` / `boolean` / `date` / `user` / `single_select` / `multi_select`), `required: bool`, `description?`, `options: Vec<String>` (for select types)

**Indexes:**
- `["project", "deletion"]` — composite index for list-by-project queries

Brief fields (list queries): `id`, `name`, `description`, `project`, `labels`, `annotations`, `state`, `acl` (ticket_types excluded)
...
| `repo_credentials` | vertex | Active |
| `crds` | vertex | Active |
| `unprocessed_images` | vertex | Active |
| `persistent_files` | vertex | Active |
| `ticketgroups` | vertex | Active (project-scoped) |
