...
Collections are auto-created on first connection via `ArangoDb::connect_basic()` in `backend/src/db/arangodb/mod.rs`. No migration files exist — schema is derived from Rust types in [models.rs](shared/src/models.rs).
